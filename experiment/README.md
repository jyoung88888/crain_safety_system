...
| **RT-DETR ResNet-50** | **`rtdetr-r50.pt`** | **Transformer 기반, NMS 불필요, 작은 객체 우수** |
| RT-DETR ResNet-101 | `rtdetr-r101.pt` | RT-DETR 고정확도 버전 |
...
라벨 포맷 (YOLO txt):
```
0 0.512 0.345 0.120 0.230    # hoist
1 0.231 0.456 0.050 0.060    # helmet
2 0.678 0.789 0.080 0.200    # person
```
...
학습이 끝나면:
- `models/<experiment_name>/weights/best.pt` → 최고 성능 모델
- MLflow에 파라미터 + 메트릭 + 모델 아티팩트 자동 기록
...
# RT-DETR 계열 실험
cp configs/exp_rtdetr.yaml configs/exp_002_rtdetr_r101.yaml
```
...
수정 예시 (`exp_002_rtdetr_r101.yaml`):
```yaml
experiment:
  name: "exp_002_rtdetr_r101"
  description: "RT-DETR ResNet-101 백본"

model:
  architecture: "rtdetr-r101.pt"
```

실행:
```bash
python src/train.py --config configs/exp_001_yolo11s.yaml
python src/train.py --config configs/exp_002_rtdetr_r101.yaml
```
...
frame_result = parse_yolo_results(results[0], class_names=["hoist", "helmet", "person"])
